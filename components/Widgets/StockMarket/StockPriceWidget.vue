        ...
        >
          ...
          <div class="flex items-center w-full p-4 text-gray-600 md:w-5/12">
            <div class="w-full">
              ...
              <span
                v-else-if="marketData.status=='Regular Trading'"
                :class="{'market-status-open-badge': marketData.status=='Regular Trading'}"
              >{{ marketData.status }}</span>
              <span
                v-else
...
export default {
  ...
  mounted: async function () {
    ...
    this.$nextTick(async function () {
      ...
      const companyData = await self.getCompanyData(self.symbolData.code)
      const candles = StockUtil.getConstantGapTimeFrameDataFromASeries(
        companyData,
        1
      )

      self.areaSeries.setData(candles)
      ...
      this.connection.onmessage = function (event) {
        ...
        if (stockDetails.data.sym == self.symbol) {
          ...
          if (stockDetails.data.ltp == undefined) {
            ...
            self.stockData = stockDetails.data
  ...
  methods: {
    ...
    getStockData: function (code) {
      const stockprice = this.generateStockPriceRequest('LKCSE', code, 0)

      this.sendMessage(stockprice)

      //TODO: need a better implementation of a isBusinesshour may be with StockMarketData
      if (StockUtil.isABusinessHour()) {
        setInterval(() => {
          this.sendMessage(stockprice)
        }, 20000)
    ...
    changeSymbols(symbol) {
      if (symbol == 'ASI') {
        return { code: 'ASPI', name: 'All Share Price Index' }
      } else {
        const indexOfCon = symbol.indexOf('`')
        if (indexOfCon == -1) {
          return { code: symbol, name: '' }
        } else {
          return { code: symbol.substring(0, indexOfCon) }
        }
      }
    },

    async getCompanyData(code) {
      try {
        const companyData = await OutsideAPI.getCompanyChartData(code, 1)

        const chartData = companyData.chartData.map((trade) => ({
          time: trade.t,
          // quantity: trade.q,
          value: trade.p,
        }))
